...
import { APP_DETAILS, USER_CATEGORY } from "../utils/constants";
    ...
    <div className="bg-white vh-100 border-right pt-2 pb-4 pl-4">
      <Link href="/" passHref>
        <div className="cursor-pointer">
          <img
            src={APP_DETAILS.APP_LOGO}
            height={50}
            width={50}
            alt="Logo ..."
            className="mr-2"
          />
          <span style={{ color: "#707070", fontWeight: "bolder" }}>
      ...
      <div className="dashboard my-3">
        <Link href={"/"} passHref>
          <div
            style={{ color: "#707070" }}
            className={
              "d-flex px-3 py-2 rounded-sm align-items-center mr-3 " +
              (pathname === "/"
      ...
      {authUser.category == USER_CATEGORY.ADMIN ? (
        <>
          <h6
            style={{ fontSize: 14 }}
            className="font-weight-light text-secondary mr-4"
          >
            MAIN MENU
          </h6>
          <div className={"main-menu " + styles.mainMenu}>
            {navList.map((list, i) => (
              <Link href={list.href} key={i} passHref>
                <div
                  className={
                    "d-flex px-3 py-2 rounded-sm align-items-center mb-2 text-secondary mr-3 " +
                    (pathname === list.href
                      ? styles.sidebarActiveLink
                      : styles.sidebarLink)
                  }
                >
                  <div style={{ marginTop: "-5px" }}>{list.icon}</div>
